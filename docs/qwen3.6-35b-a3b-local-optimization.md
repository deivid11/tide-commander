...
El servidor quedó activo con estos datos. Desde el 17 de agosto corre como
servicio systemd de usuario (`llama-server.service`, ver Próximos pasos) y se
administra con `llama-profile`:
...
### Tercera ronda de quants: IQ4-MIX (17 de agosto)

A raíz de dos posts de la comunidad (un RTX 5060 Ti 16 GB a 47 tok/s con
MTP-2 y un usuario AMD a 40 tok/s con 78-86K de contexto usando
`vmarcelo/Qwen3.8-27B-MIX_GGUF`), se probó el quant `IQ4-MIX` (13.14 GiB:
IQ4_XS con capas selectas en IQ3_S, un "dinámico" a mano) y el `Q4_K_M` de
unsloth que David pidió medir:

| Quant | Modo | Gen tok/s | VRAM proceso | Nota |
| --- | --- | ---: | ---: | --- |
| unsloth Q4_K_M (16 GiB) | híbrido 40/64 GPU | 12.3 | 11.5 GB | no cabe todo-en-GPU |
| unsloth Q4_K_M | híbrido 46/64 GPU | 14.3 | 12.9 GB | techo del híbrido |
| UD-IQ3_XXS + receta AMD (ub128, KV q5_1, MTP2 p0.82) | todo-GPU 32K | 27.0 | 12.7 GB | la receta EMPEORA nuestro quant |
| **IQ4-MIX + nuestra receta (ub256, KV q4_0, MTP4 p0.7)** | **todo-GPU 32K** | **42-45** | **15.1 GB** | ganador; acc 0.72; prefill 538 @4K |
| IQ4-MIX + receta AMD | todo-GPU 64K | 35.2 | 15.6 GB | acc 0.90 pero más lento |
| IQ4-MIX + nuestra receta | todo-GPU 48K | 40.1 | 15.6 GB | VRAM al filo |
| IQ3-MIX (11.7 GiB) | todo-GPU 32K | 30.1 | 13.5 GB | peor que UD-IQ3_XXS |

Conclusiones: (1) la ganancia es del **quant**, no de la receta del post — el
IQ4-MIX rinde 42-45 tok/s (+30% sobre los 32.7 del UD-IQ3_XXS) y con más bits
(mejor calidad esperada); (2) su peso en runtime es 15.1 GB aunque el archivo
mida 13.1 GiB: deja ~1.5 GB al escritorio y ~2.5 GB desbordados a GTT.
Estable en un test de 800 tokens sostenidos (40.3 tok/s, cero colapsos de
`tg_3s`), pero es la config más apretada de todas — con navegador muy pesado
pueden volver los congelones transitorios; (3) los 64-86K de contexto del post
AMD sí caben (probado 64K a 35 tok/s) a costa de la VRAM total; se dejó 32K
por convivencia.

**Perfil adoptado en `start-qwen38-desktop.sh`: IQ4-MIX, ubatch 256, ctx
32K, KV q4_0, MTP 4 / p-min 0.7, reasoning medium.** GGUFs conservados en
`../.cache/llama-opt/models/`: IQ4-MIX (activo), UD-IQ3_XXS (fallback ligero,
12.9 GB, 32.7 tok/s) e IQ4_XS (híbrido de calidad, 11 tok/s). Borrados:
Q4_K_M e IQ3-MIX.

### Contexto largo en el denso: 100-200K sí son viables (17 de agosto)

Corrección importante a la aritmética anterior: el Qwen 3.8-27B es **híbrido**
— solo 16 de sus 64 capas tienen atención completa (con KV cache); las otras
48 son Gated DeltaNet con estado recurrente de tamaño fijo (verificado en el
GGUF: 68 tensores `attn_k` frente a 192 `ssm_*`, y la clave
`full_attention_interval`). El KV real cuesta ~18 KiB/token, no los 72 que se
habían asumido, y por eso 100K y 200K entran en el presupuesto de la máquina.

Lo que NO cabe es todo-en-GPU: con el escritorio ocupando ~3 GB de VRAM, 80K
y 128K fallaron al crear el contexto (el post AMD que llegó a 84K en ROCm
seguramente corría headless). La palanca es `--no-kv-offload`: los pesos y el
estado recurrente quedan en VRAM y el KV de las 16 capas de atención vive en
la RAM del sistema. Mediciones con IQ4-MIX:

| Perfil | Ctx | Gen (ctx vacío) | Gen (~30K llenos) | Prefill @30K | VRAM proc |
| --- | ---: | ---: | ---: | ---: | ---: |
| todo-en-GPU (`start-qwen38-desktop.sh`) | 32K | 42-45 | 38.0 | 432 tok/s (65 s) | 15.1 GB |
| KV en RAM (`start-qwen38-longctx.sh`) | 128K | 20.3 | — | — | 14.8 GB |
| KV en RAM (`start-qwen38-longctx.sh`) | **200K** | **20.5** | **14.7** | 304 tok/s (104 s) | 15.5 GB |

La atención lee el KV por PCIe en cada token, así que la generación baja
conforme se llena el contexto (14.7 tok/s a 30K; a 100K+ estimado 8-11); el
prefill también es más lento (304 vs 432). Es el precio de 6× más contexto:
usable para análisis de bases de código y documentos largos, no para chat
ágil. Los dos scripts comparten el puerto 8080 y se alternan según la tarea.
### Sweep MTP profundo y A/B ROCm vs Vulkan (17 de agosto, noche)

Motivación: `julianmb/q38rocm` reporta en un Strix Halo (RDNA 3.5) ratios MTP
de 2.5-2.9× con drafts profundos y permisivos (n6-7, p-min 0.35-0.6) y decode
casi 2× más rápido en Vulkan (RADV coopmat Wave64) que en ROCm. Se replicó en
la RX 7800 XT con el harness `bench/sweep.sh` (IQ4-MIX, 32K, mismo commit
89e0aa6 en ambos builds, dos prompts: código y prosa, 350 tokens):

| Backend | MTP | Código | Prosa | Acc (código/prosa) |
| --- | --- | ---: | ---: | --- |
| ROCm | sin MTP | 26.7 | 26.6 | — |
| Vulkan | sin MTP | 17.7 | 17.7 | — |
| **ROCm** | **n4 p0.7 (actual)** | **50.4-51.3** | **37.9-38.1** | 0.78-0.81 / 0.63-0.69 |
| Vulkan | n4 p0.7 | 43.9 | 39.1 | 0.74 / 0.66 |
| ROCm | n5 p0.5 | 45.4 | 39.1 | 0.58 / 0.51 |
| ROCm | n5 p0.7 | 47.1 | 38.6 | 0.69 / 0.72 |
| ROCm | n6 p0.6 | 44.5-46.9 | 36.7-40.8 | 0.59-0.64 / 0.51-0.62 |
| ROCm | n6 p0.35 | 39.1 | 31.4 | 0.44 / 0.32 |
| ROCm | n7 p0.35 | 43.1 | 30.1 | 0.50 / 0.31 |
| ROCm | n8 p0.35 | 47.1 | 33.5 | 0.45 / 0.32 |

Conclusiones: (1) en RDNA3 discreto **Vulkan/RADV pierde en decode puro**
(17.7 vs 26.7: no tiene la ventaja coopmat Wave64 del RDNA 3.5 integrado);
ROCm sigue siendo el backend; (2) la receta "profunda y permisiva" del Strix
Halo **empeora** aquí — p-min ≤0.35 hunde la aceptación a 0.3-0.45 y la
prosa cae a 30-33; (3) **n4/p0.7 se confirma óptimo** y repetible (51/38);
la variación entre corridas es ±2-3 tok/s. El ratio MTP real en esta GPU es
...
1. ~~Crear una unidad de usuario `systemd`~~ **Hecho el 17 de agosto**:
   `~/.config/systemd/user/llama-server.service` (enabled, linger activo,
   `Restart=on-failure`, `Nice=5`, `CPUWeight=50`, `MemoryHigh=40G`,
   `MemoryMax=52G`, log en `llama-server-service.log`). El perfil se lee de
   `tc-playground/nemotron-desktop-profile/profile.conf` vía
   `llama-server-run.sh`; se cambia con el helper `llama-profile <nombre>`
   (`list` / `status`; en `~/.local/bin`). Perfiles: `qwen38-longctx` (activo,
   200K), `qwen38-desktop` (32K, 42-45 tok/s), `qwen-desktop` (3.6 MoE),
   `nemotron-desktop`. Verificado: `kill -9` → resucita en ~20 s. Los scripts
   `start-*.sh` siguen funcionando a mano, pero el wrapper mata cualquier
   `llama-server` huérfano de un nohup antes de arrancar.
