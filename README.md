...
- Pi agents run `pi --mode json -p …` (headless NDJSON) and resume via `--session <sessionId>`; the optional Pi RPC mode (Config → General) keeps a persistent `pi --mode rpc` process per agent and STEERS mid-turn messages into the live run instead of queueing them. With tmux mode enabled, Pi RPC processes survive Commander restarts and reconnect automatically
