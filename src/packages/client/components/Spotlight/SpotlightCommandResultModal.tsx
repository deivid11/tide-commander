...
export function SpotlightCommandResultModal({
  ...
  const invokesAgent = Boolean(
    output.data
    && typeof output.data === 'object'
    && !Array.isArray(output.data)
    && (output.data as { kind?: unknown }).kind === 'agent-name-proposals'
  );
    ...
    >
      <section className="spotlight-command-result-modal" role="dialog" aria-modal="true" aria-label={`Result for ${command}`}>
        ...
        <footer className="spotlight-command-result-footer">
          <span><Icon name={invokesAgent ? 'brain' : 'lock'} size={11} /> {invokesAgent ? 'Orchestrated by the plugin' : 'Executed locally by the plugin'}</span>
          <span>{invokesAgent ? 'Proposals generated by the selected agent' : 'The LLM was not invoked'}</span>
        </footer>
      </section>
    </div>
  );
